...
function rePag() {
    cy.task("logCommand", { funtype: "Attempt rePag" });
    let info = ""
    let prev = curY.valueOf()
    if (curY === 0) {
...
function horizontalScrollFw() {
    cy.task("logCommand", { funtype: "Attempt horizontalScrollFw" });
    let info = ""
...
function horizontalScrollBk() {
    cy.task("logCommand", { funtype: "Attempt horizontalScrollBk" });
    let info = ""
    let prev = curX.valueOf()
    if (curX === 0) {
