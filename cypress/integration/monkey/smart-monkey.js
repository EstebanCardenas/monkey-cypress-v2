...
/**
 * Este tipo de evento está asociado a reloads.
*/
...
function randDClick() {
    ...
    cy.window().then((win) => {
        ...
        if (!!element) {
            //Use cypress selector if any fits
            if (!!element.id) { //boolean that indicates if the element has a non-empty id
                cy.task("logCommand", { funtype: "Attempt randDClick-1 dblclick" });
            ...
            /*
                ...
                cy.get(jsPath).then($candidates => {
                    ...
                    for (let i = 0; i < $candidates.length; i++) {
                        let candidate = $candidates.get(i)
                        if (!Cypress.dom.isHidden(candidate)) {
                            cy.task("logCommand", { funtype: "Attempt randDClick-2 dblclick" });
                            cy.wrap(candidate).dblclick({ force: true })
                            break
                        }
                    }
            ...
            cy.task("logCommand", { funtype: "Attempt randDClick-3 dblclick" });
...
function avPag() {
    ...
    if (curPageMaxY - curY >= viewportHeight) {
        if (curPageMaxY - (curY + viewportHeight) >= viewportHeight) {
            ...
            if (curY !== 0 && curX !== 0) cy.scrollTo(curX, curY)
        }
        else {
...
function enter() {
    cy.task('logCommand', { funtype: 'Attempt Enter' });
    let info = ""
    if (focused) {
...
function typeCharKey() {
    cy.task('logCommand', { funtype: 'Attempt type char key' });
    let info = ""
...
function spkeypress() {
    cy.task('logCommand', { funtype: 'Attempt spkeypress' });
    let info = ""
        ...
        if ($inputs.length > 0) {
            ...
            if (!Cypress.dom.isHidden(inp)) {
                ...
                if (inp.getAttribute("type") == "email") {
                    cy.task("logCommand", { funtype: "Attempt fillInput email" });
